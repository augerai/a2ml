...
class AzureA2ML(object):
    def __init__(self, ctx):
        ...
        self.metric = azure_cfg.get(
            'experiment/metric', 'spearman_correlation')
        self.cross_validation_folds = azure_cfg.get(
            'experiment/cross_validation_folds', 5)
        self.max_total_time = cfg.get('experiment/max_total_time', 60)
        self.iteration_timeout_minutes = cfg.get(
            'experiment/iteration_timeout_minutes', 10)
        self.max_n_trials = cfg.get('experiment/max_n_trials', 10)
        self.use_ensemble = cfg.get('experiment/use_ensemble', False)

        # per provider compute settings
        self.subscription_id = azure_cfg.get(
            'subscription_id', os.environ.get("AZURE_SUBSCRIPTION_ID"))
        self.workspace = azure_cfg.get('workspace', self.name+'_ws')
        self.resource_group = azure_cfg.get(
            'resource_group', self.name+'_resources')

        # cluster specific options
        self.compute_cluster = azure_cfg.get('cluster/name', 'cpucluster')
        self.compute_region = azure_cfg.get('cluster/region', 'eastus2')
        self.compute_min_nodes = azure_cfg.get('cluster/min_nodes', 0)
        self.compute_max_nodes = azure_cfg.get('cluster/max_nodes', 4)
        self.compute_sku = azure_cfg.get('cluster/type', 'STANDARD_D2_V2')
        # azure-specific file-related options
        self.file_share = azure_cfg.get('file_share', None)
        self.account_name = azure_cfg.get('account_name', None)
        self.account_key = azure_cfg.get('account_key', None)
        # example: https://autoautostorage68c87c828.file.core.windows.net/a2ml/baseball.csv
        default_data_file = "https://{}.file.core.windows.net/{}/{}".format(
            self.account_name,
            self.file_share,
            os.path.basename(self.source))
        self.data_file = azure_cfg.get('file_share', default_data_file)
        ...
        except Exception:  # or create a new one
        ...
        if self.compute_cluster in self.ws.compute_targets:
            ...
            if compute_target and type(compute_target) is AmlCompute:
                self.ctx.log('Found compute target. Just use it: ' +
    ...
    def upload_file(self, source_file):
        ...
        cmd = ("az storage file upload --source " + self.source +
               " --share-name " + self.file_share +
               " --account-name " + self.account_name +
               " --account-key " + self.account_key)
        self.ctx.log("Running command: {}".format(cmd))
        result = os.system(cmd)
        self.ctx.log(
            "Result of local copy to Azure file share: {}".format(result))
    ...
    def deploy(self,  model_id, locally=False):
        ...
        self.exp = Experiment(workspace=self.ws, name=self.name)
        model = exp.get_output()
        self.ctx.log('Deploy model: {}'.format(self.model))
