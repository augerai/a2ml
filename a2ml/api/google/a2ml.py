...
class GoogleA2ML(object):
    ...
    def __init__(self, ctx):
        ...
        self.compute_region = ctx.config['google'].get(
            'cluster/region', 'us-central1')
        self.metric = ctx.config['google'].get(
            'experiment/metric', 'MINIMIZE_MAE')
        self.project_location = self.client.location_path(
            self.project_id, self.compute_region)
        ...
        self.dataset_name = self.client.dataset_path(
    ...
    def import_data(self):
        ...
        self.ctx.log('Creating dataset for project location: {}'.format(
        ...
        if os.path.isfile(self.source):
            ...
            cmd = 'gsutil cp {} {}'.format(self.source, self.gsbucket)
            result = os.system(cmd)
            print(
                'Result of local copy to Google Storage bucket: {}'.format(
                    result)
            )
        ...
        self.ctx.log(
            'Processing import for dataset ID: {}'.format(self.dataset_id))
    ...
    def train(self, synchronous=False):
        ...
        list_table_specs_response = self.client.list_table_specs(
            self.dataset_name)
        # self.ctx.log('List table specs response: {}'.format(
        ...
        list_column_specs_response = self.client.list_column_specs(
            table_spec_name)
        self.column_specs = {
            s.display_name: s for s in list_column_specs_response}
        ...
        update_dataset_response = self.client.update_dataset(
            update_dataset_dict)
        self.ctx.log('Updated dataset response: {}'.format(
        ...
        model_dict = {
            ...
            'tables_model_metadata': {
                ...
                'train_budget_milli_node_hours': self.max_total_time/60*1000,
                # budget is in minutes, google wants 'millihours', seriously?
    ...
    def evaluate(self):
        ...
        if (('done' in result.keys()) and result['done']):
            ...
            for evaluation in response:
                self.ctx.log(
                    'Model evaluation name: {}'.format(evaluation.name))
                self.ctx.log(
                    'Model evaluation id: {}'.format(
                        evaluation.name.split('/')[-1]))
                self.ctx.log('Model evaluation example count: {}'.format(
                    evaluation.evaluated_example_count))
                self.ctx.log('Model evaluation time: {} seconds'.format(
                    evaluation.create_time.seconds))
                self.ctx.log('Full model evaluation: {}'.format(
                    inspect.getmembers(evaluation)))
                self.ctx.log('\n')
        else:
    ...
    def deploy(self, model_id, locally=False):
        ...
        # TODO: local model_name assigned but never used
        if (model_id is None):
            model_name = self.model_name
        else:
            model_name = self.client.model_path(
                self.project_id, self.compute_region, self.id)
        try:
            # TODO:
            # seems to be misused self.model_name instead of local model_name
            ...
            self.ctx.log(
                'Failed to deploy because its already deploying: {}...'.format(
                    inst))
    ...
    def review(self):
        ...
        self.ctx.log('Model create time (seconds): {}'.format(
            model.create_time.seconds))
